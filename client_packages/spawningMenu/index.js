let browser = mp.browsers.new("package://spawningMenu/index.html")
...
// CEF baut alles auf und macht es dann unsichbar
browser.execute("replaceall();");


var showing = false;

// Umschalten der anzeige durch CEF
mp.keys.bind(bindVirtualKeys.F3, true, function() {
	if(!showing){
        browser.execute("displayMe();");
        showing = true;
    } else {

        browser.execute("hideMe();");
        showing = false;
    }
});
...
function replaceall(){
    ...
    let inhalt = "<center><h1>SpawnMenü</h1>  <h3>Kosten pro Vehicle: 100$ </h3>";
    inhalt += '<input type="text" id="carSearch" onkeyup="searchFunction()" placeholder="Suche nach Fahrzeug">';
    inhalt += "</center>";
    inhalt += '<ul id="carUL">';
    for(let [key,value] of Object.entries(vehicleHashes)){


        inhalt = inhalt + '<li>'+'<a onClick="spawnMe('+ "'" +key + "'" +')" class="name">' + '<img src="veh/' +key+ '.png " >' + capitalizeFirstLetter(key) + '<br></a></li> ';
    
    }
    inhalt += '</ul>';


    document.getElementById("all").innerHTML = inhalt;
...
function searchFunction() {
    ...
    ul = document.getElementById("carUL");
    li = ul.getElementsByTagName('li');

    // Durch alle Items gehen und falsche rauslöschen
...
// CEF zum verstecken des Menüs
function hideMe(){
    document.getElementById("all").style.display = "none";
    mp.trigger("cursorOff");
}

// Cursor verstecken und chat aktivieren
...
// Cursor anzeigen und Chat deaktivieren
...
// Normale js Funktion in CEF
function spawnMe(veh) {
    // CEF -> Client
    mp.trigger("spawnVeh",veh);
    hideMe();
}

// Rage mp funktion im Client
mp.events.add("spawnVeh",(veh) => {
    // Client -> Server
    mp.events.callRemote("spawnVehServer",veh)
